    ...
    public class AnimateActor : Command
    {
        ...
        public override async UniTask ExecuteAsync (CancellationToken cancellationToken = default)
        {
            ...
            foreach (var actorId in ActorIds)
                tasks.Add(AnimateActorAsync(actorId, cancellationToken));
            await UniTask.WhenAll(tasks);
        }

        protected virtual async UniTask AnimateActorAsync (string actorId, CancellationToken cancellationToken)
        {
            var spawnPath = BuildSpawnPath(actorId);
            var spawnedObject = await SpawnManager.GetOrSpawnAsync(spawnPath, cancellationToken);
